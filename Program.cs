...
namespace batalha_naval
{
    class Program
    {
        ...
        public static string TipoDaEmbarcacao(List<int> lista)
        {
            ...
            var validaCoordenadas = new Tuple<bool, string[]>(false, teste);
            string escolha;
            do
            {
            ...
            if(validaQuantidade)
            {
                validaCoordenadas = VerificaCoordenadas();
            }

            if(validaCoordenadas.Item1)
            {
                VerificaTamanho(escolha, validaCoordenada.Item2);
            }
            
            }while(!validador && validaCoordenadas.Item1);
            return escolha;
        }

        public static Tuple<bool, string[]> VerificaCoordenadas()
        { 
            ...
            {
                if(!letras.Contains(coordenadas[0]) || !letras.Contains(coordenadas[2])) return new Tuple<bool, string[]>(false, comecoEfim);;

                if(!numeros.Contains(coordenadas[1]) || !numeros.Contains(coordenadas[3])) return new Tuple<bool, string[]>(false, comecoEfim);;

                comecoEfim[0] = coordenadas.Substring(0,2);
                comecoEfim[1] = coordenadas.Substring(2,4);

                var resultado = new Tuple<bool, string[]>(true, comecoEfim);
                return resultado;
            }

            if(coordenadas.Length == 6)
            {
                if(!letras.Contains(coordenadas[0]) || !letras.Contains(coordenadas[3])) return new Tuple<bool, string[]>(true, comecoEfim);

                if(!numeros.Contains($"{coordenadas[1]}{coordenadas[2]}") || !numeros.Contains($"{coordenadas[4]}{coordenadas[5]}")) return new Tuple<bool, string[]>(true, comecoEfim);
            ...
            {   
                ...
                {
                    if(!letras.Contains(coordenadas[0])) return new Tuple<bool, string[]>(true, comecoEfim);
                    if(!numeros.Contains(coordenadas[1]) || !numeros.Contains($"{coordenadas[3]}{coordenadas[4]}")) return new Tuple<bool, string[]>(true, comecoEfim);
                ...
                {
                    if(!letras.Contains(coordenadas[0]) || !letras.Contains(coordenadas[3])) return new Tuple<bool, string[]>(true, comecoEfim);
                    if(!numeros.Contains($"{coordenadas[1]}{coordenadas[2]}") || !numeros.Contains(coordenadas[4])) return new Tuple<bool, string[]>(true, comecoEfim);
                ...
                {
                    return new Tuple<bool, string[]>(true, comecoEfim);;
                }
            }

            var res = new Tuple<bool, string[]>(false, comecoEfim);
            return res;
        }

        public static bool VerificaTamanho(string escolha, string[] comecoEfim)
        {
